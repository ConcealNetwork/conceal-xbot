...
function getAllChannelUsers(client, usersData, count, channelId, authorId) {
  return new Promise((resolve, reject) => {
    let pureChannelId = channelId.substring(channelId.indexOf('<') + 1, channelId.lastIndexOf('>'));
    ...
    if (channel.members.size > 0) {
      channel.members.every((member) => {
        if (
          ...
          !member.user.bot &&
          !checkUserRoleIsExcluded(member, config.rain.exclusions.channel)
        ) {
...
function getAllRoleUsers(roles, usersData, count, roleId, authorId) {
  return new Promise((resolve, reject) => {
    let pureRoleId = roleId.substring(roleId.indexOf('<') + 1, roleId.lastIndexOf('>'));
  ...
  executeCommand: async function (
    ...
    if (
      ...
      try {
        ...
        if (!amount || amount < config.restrictions.minRainAmount)
          throw `Amount cannot be less then ${config.restrictions.minRainAmount} CCX`;
      } catch (err) {
        return message.reply(err);
      }
              ...
              if (balanceData.balance > amount * config.metrics.coinUnits) {
                ...
                (async () => {
                  ...
                  if (payments.length > 0) {
                    ...
                    message.author.send(
                      `Success! ***TX hash***: ${txdata.transactionHash}\n***Secret key***: ${txdata.transactionSecretKey}`
                    );
                    ...
                    for (let i = 0; i < chunks; i++) {
                      ...
                      message.channel.send(
                        `\:money_with_wings: ${payPart.toFixed(6)} CCX rained on users ${chunkUsers.join()}`
                      );
                    }
                  } else {
