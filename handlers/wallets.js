...
module.exports = {
  executeCommand: function (walletsData, message, command, args) {
    function sendCommonError(errMsg) {
      ...
      message.author.send(
        'If you do not have the wallet registered yet please register it. For list of wallet commands type ```.wallet help```'
      );
    }
    ...
    if (availableCommands.indexOf(args[0]) == -1) {
      ...
      return message.channel.send(
        'Unknown wallet command. Type ".wallet help" for available commands'
      );
    }
    ...
    if (args[0] == 'register') {
      if (args.length < 2) {
        ...
        walletsData
          .registerWallet(message.author.id, message.author.username, args[1])
          .then((response) => {
          ...
          .finally(() => {
            sendNotification('The registration information has been sent to you in DM');
          });
      }
    }
    ...
    if (args[0] == 'update') {
      if (args.length < 2) {
        ...
        walletsData
          .updateWallet(message.author.id, args[1])
          .then((response) => {
            message.author.send(response);
          })
          .catch((err) => {
          ...
          .finally(() => {
            sendNotification('The update information has been sent to you in DM');
          });
      }
    }
    ...
    if (args[0] == 'show') {
      walletsData
        .showWalletInfo(message.author.id)
        .then((data) => {
        ...
        .finally(() => {
          sendNotification('The wallet information has been sent to you in DM');
        });
    }
    ...
    if (args[0] == 'deposit') {
      walletsData
        ...
        .finally(() => {
          sendNotification('The deposit information has been sent to you in DM');
        });
    }
    ...
    if (args[0] == 'balance') {
      walletsData
        ...
        .then((data) => {
          message.author.send(
            `***Balance***: ${(data.balance / config.metrics.coinUnits).toLocaleString()} CCX`
          );
        })
        .catch((err) => {
        ...
        .finally(() => {
          sendNotification('The balance information has been sent to you in DM');
        });
    }
    ...
    if (args[0] === 'paymentid') {
      walletsData
        ...
        .finally(() => {
          sendNotification('The paymentid information has been sent to you in DM');
        });
    }
  },
};
