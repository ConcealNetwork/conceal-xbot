...
let availableCommands = ['help', 'all', 'online', 'offline', 'kick', 'ban', 'list'];
  ...
  executeCommand: async function (usersData, client, message, command, args) {
    if (availableCommands.indexOf(args[0]) == -1) {
      ...
      return message.channel.send(
        'Unknown users command. Type ".users help" for available commands'
      );
    }
    ...
    if (args[0] === 'list') {
      if (args[1] === 'alltime' || args[1] === 'period' || args[1] === 'tippers') {
        function printUsernames(users) {
          (async () => {
            ...
            for (let i = 0; i < users.length; i++) {
              let dUser =
                client.users.cache.get(users[i].user_id) ||
                (await client.users.fetch(users[i].user_id));
              if (i == 0) {
        ...
        return message.channel.send(
          'Uknows list command. Type ".users help" for available commands'
        );
      }
    }
  },
