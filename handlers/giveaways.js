...
let availableCommands = ['help', 'create', 'list', 'delete'];
  ...
  executeCommand: async function (giveawaysData, walletsData, client, message, command, args) {
    if (availableCommands.indexOf(args[0]) == -1) {
      ...
      return message.channel.send(
        'Unknown giveaway command. Type ".giveaway help" for available commands'
      );
    }
    ...
    if (args[0] === 'create') {
      ...
      if (!args[3]) {
        ...
        try {
          ...
          if (!amount || amount < config.restrictions.minGiveawayAmount)
            throw `Amount cannot be less then ${config.restrictions.minGiveawayAmount} CCX`;
        } catch (err) {
          return message.reply(err);
        }
      }
      ...
      walletsData
        .userHasWallet(message.author.id)
        .then((hasWallet) => {
          if (hasWallet) {
            walletsData
              ...
              .then((balanceData) => {
                if (balanceData.balance > amount * config.metrics.coinUnits) {
                  ...
                  const giveawayEmbed = giveawaysData.createEmbedMessage(
                    title,
                    description,
                    footer
                  );
                        ...
                        .then((data) => {
                          ...
                          if (!isSameChannel) {
                            message.channel.send(
                              `The giveaway was created in <#${config.giveaways.channel}>`
                            );
                          }
                        })
                        .catch((err) => {
    ...
    if (args[0] === 'delete') {
      if (!args[1]) {
        ...
        giveawaysData
          .getGiveawayByRowId(giveawayId)
          .then((giveawayData) => {
            if (
              giveawayData.user_id !== message.author.id &&
              !message.member.roles.cache.some((r) => ['admins', 'mods'].includes(r.name))
            ) {
        ...
        (async () => {
          ...
          if (validUsers.length > 0) {
            ...
            for (let i = 0; i < winners.length; i++) {
              let isMuted = true;
              try {
                isMuted = await settingsData.getMutedState(winners[i].id);
              } catch (err) {
                console.log(err);
              }
              payments.push({
            ...
            fs.readFile('./templates/giveaway_finished.msg', 'utf8', function (err, source) {
              ...
              const gaEmbed = giveawaysData.createEmbedMessage(
                finishedData.description,
                embedDescription,
                footerText
              );
              message.delete().catch((O_o) => {});
