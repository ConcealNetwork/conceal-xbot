...
let availableCommands = ['help', 'mute'];
  ...
  executeCommand: async function (settingsData, message, command, args) {
    if (availableCommands.indexOf(args[0]) == -1) {
      ...
      return message.channel.send(
        'Unknown settings command. Type ".settings help" for available commands'
      );
    }
    ...
    if (args[0] === 'mute') {
      if (!args[1]) {
        ...
        if (args[1] === 'on') {
          settingsData
            .setMutedState(message.author.id, true)
            .then(() => {
          ...
          settingsData
            .setMutedState(message.author.id, false)
            .then(() => {
