...
let db = new sqlite3.Database(
  path.join(appRoot.path, config.db.filename),
...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'exchanges') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a exchanges command! Type: ***${config.prefix}exchanges help*** for list of commands`
      );
    }
  ...
  if (command === 'markets') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a markets command! Type: ***${config.prefix}markets help*** for list of commands`
      );
    }
  ...
  if (command === 'blockchain' || command === 'chain') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a blockchain command! Type: ***${config.prefix}blockchain help*** for list of commands`
      );
    }
  ...
  if (command === 'wallet') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a wallet command! Type: ***${config.prefix}wallet help*** for list of commands`
      );
    }
  ...
  if (command === 'pools') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a pool command! Type: ***${config.prefix}pools help*** for list of commands`
      );
    }
  ...
  if (command === 'giveaway') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a giveaway command! Type: ***${config.prefix}giveaway help*** for list of commands`
      );
    }
  ...
  if (command === 'users') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a users command! Type: ***${config.prefix}users help*** for list of commands`
      );
    }
  ...
  if (command === 'settings') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a settings command! Type: ***${config.prefix}settings help*** for list of commands`
      );
    }
  ...
  if (command === 'tip') {
    ...
    try {
      ...
      if (!amount || amount < config.restrictions.minTipAmount)
        throw `Amount cannot be less then ${config.restrictions.minTipAmount} CCX`;
    } catch (err) {
    ...
    // execute the blockchain commands
    return walletsData
      .sendPayments(message.author.id, payments)
      .then((data) => {
        message.author.send(
          `Success! ***TX hash***: ${data.transactionHash}\n***Secret key***: ${data.transactionSecretKey}`
        );
  ...
  if (command === 'rain') {
    if (args.length == 0) {
      return message.reply(
        `You need to specify a rain command! Type: ***${config.prefix}rain help*** for list of commands`
      );
    }

    // execute the rain commands
    return rains.executeCommand(
      usersData,
      walletsData,
      settingsData,
      client,
